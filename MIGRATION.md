...
- **Exception unique — les données IA en lecture** : le fichier d'état `docs/data/history.json` est publié par GitHub Pages et constitue une **interface publique de lecture seule**, librement consommable par des applications tierces. AnaData ne tient pas de registre de ses consommateurs et **ne les mentionne nulle part** ; en contrepartie, le schéma du fichier est un **contrat stable** — évolutions rétro-compatibles uniquement (ajout de champs autorisé ; ne jamais supprimer, renommer ou changer le type d'un champ existant sans l'annoncer dans le README).
